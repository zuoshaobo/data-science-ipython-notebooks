...
def get_model():
    ...
    #'''
    up6 = merge([UpSampling2D(size=(3, 3))(conv4), conv3], mode='concat', concat_axis=3)
    conv6 = Conv2D(64, (3, 3), activation='relu', padding='same')(up6)
    conv6 = Conv2D(64, (3, 3), activation='relu', padding='same')(conv6)

    up8 = merge([UpSampling2D(size=(2, 2))(conv6), conv2], mode='concat', concat_axis=3)
...
while 1:
    ...
    print y.shape
'''
filepath="line.best.hdf5"
checkpoint = ModelCheckpoint(filepath, monitor='loss', verbose=1, save_best_only=True, mode='min')
callbacks_list = [checkpoint]
model.fit_generator(generate_arrays_from_file(''),
                steps_per_epoch=559,epochs=nb_epoch,verbose=1,callbacks=callbacks_list)
model.save('line.h5')
